...
// Inline icon set — 20px line icons on a 24 grid, stroke currentColor
// (catalog `Icon`). Usage: <Icon name="plus" />. Config-driven surfaces
// (sidebar items, menus) reference icons by IconName string.
...
const paths: Record<string, ReactNode> = {
  ...
  "calendar-check": (
    <>
      ...
      <polyline points="9 15.5 11 17.5 15 13.5" />
    </>
  ),
  "file-up": (
    <>
  ...
  "paint-roller": (
    <>
      ...
      <path d="M18 6h2a2 2 0 0 1 2 2v2.5a2 2 0 0 1-2 2h-8.5V15" />
      <rect x="10.25" y="15" width="2.5" height="7" rx="1" />
    </>
  ),
  "log-out": (
    <>
  ...
  lock: (
    <>
      ...
      {/* shackle: never fills (stays a line even when the box is filled) */}
  ...
  // Lucide `star` — closed polygon so it two-tones (fill) on hover.
  star: (
    <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" />
  ),
  // Lucide `pin` — pin-to-top affordances (directory row menu).
  pin: (
    <>
      <path d="M12 17v5" />
  ...
  // Lucide `wand-sparkles` — the wand body is a closed path so it two-tones (fill) on hover.
  "wand-sparkles": (
    <>
...
/** Catalog `IconSquare` — a 20px icon centered in a 36px grey rounded square. */
export function IconSquare({ name, className = "" }: { name: IconName; className?: string }) {
  return (
    <span
      className={`inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-field bg-[#F3F4F6] text-text-body ${className}`}
    >
      <Icon name={name} />
    </span>
  );
