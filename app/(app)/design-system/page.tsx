...
• HEADER MENUS + THE TABLE'S POWER SET (NYS-147): every DataTable header is a MENU — Sort asc/desc, Filter by value (a column declares filterValue=; active filters render as clearable toolbar chips), Hide column. Right-click still opens the column picker. groupBy= turns the table into the collapsible count-header TREE (e.g. Insurer → Network, same columns top to bottom; the Group chip flattens / re-applies it). selected= + bulkActions= (compose BulkAction) raise the floating navy bulk bar ("N selected · … · ×"). A missing value NEVER renders blank — EmptyCell gives it a semantic label ("Insurer-run", "No rates") or makes the empty cell the CTA that fills it. Reference implementation: /networks.
