...
//     data dictionary) and the /dashboard + /workspace Observatory. It layers
...
const INSIGHTS = { href: "/workspace", label: "Workspace" };
...
export const TABLE_GROUPS = [
  ...
  },
  {
    ...
    tables: [
      {
        name: "insurers",
        meaning: "Canonical insurer entities (DFS-resolved, sql/043) — the fixed points every payer label maps onto.",
        links: "id → networks.insurer_id",
        sql: "sql/043",
        powers: null,
        keys: ["insurer_id"],
        joins: ["networks"],
        blurb: "The canonical insurer layer — one entity per real insurer, whatever a file calls it.",
      },
      {
        name: "networks",
        meaning: "Canonical networks + label crosswalk (sql/044, resolves 99.35% of rate rows) — the join point where FHIR and MRF vocabularies meet.",
        links: "insurer_id → insurers.id",
        sql: "sql/044",
        powers: null,
        keys: ["network_id", "insurer_id"],
        joins: ["insurers", "org_network_rates"],
        blurb: "The canonical network layer — directory labels and rate labels join through it.",
      },
      {
        name: "payer_sources",
